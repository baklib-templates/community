...
- **员工评论免审：** 组织员工账号（有部门 ID，与日发帖限额判定一致）回复始终即时发布，不受「评论先发后审」开关限制；访客账号仍跟随站点设置。
...
- **审核机制**：帖子与评论可分别配置先发后审或先审后发；组织员工回复始终即时发布
- **响应式设计**：适配桌面端和移动端
...
└── snippets/ …（含 reply_published_flag 员工评论免审）
```
...
2. 配置站点设置（LOGO、默认头像、**帖子/评论审核模式**等）。版主前台账号请使用带部门的组织员工，以便评论免审与日限额豁免一致。
